...
module Pages
  def bws_items_tabs
    @bws_items_tabs ||= BwsItemsTabs.new
  end

  class BwsItemsTabs < TestEvolve::Core::PageObject
    ...
    element(:bws_ok) { span(text: 'Cancel') }
    element(:submit_the_buy) { span(text: 'Submit the Buy') }


    ## elements ##
    element(:element_check) { |text| text_field(label: text) }
    ...
    def fields_item_bws(sub_Department, category, sub_category, main_Desc, marketing_Desc, differentiator_1,
